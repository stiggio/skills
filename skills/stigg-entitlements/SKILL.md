...
description: Use when implementing the runtime side of Stigg — gating feature access (`hasAccess`, `getMeteredEntitlement`, `getBooleanEntitlement`), reporting usage (raw events vs calculated measurements), and configuring local caching / fallback so checks stay reliable when Stigg is unreachable. Triggers include "gate this feature with Stigg", "check entitlement", "report usage to Stigg", "report a usage event", "events.report vs usage.report", "Stigg fallback strategy", "Stigg cache", "Stigg sidecar", "Stigg goes down". Skip for catalog modeling (use stigg-pricing-modeling) and subscription lifecycle (use stigg-subscriptions).
---
