# Assignments — Budgets per (Entity, Feature or Credit Currency)

An assignment is the budget itself: a usage limit for one entity on one feature or credit currency. Field names below reflect the surface at the time of writing — **confirm shapes via the Stigg MCP's `search_docs` before calling.**

## Shape

- `entityId` — The entity being budgeted (any level of the customer's tree).
- `parentId` — The entity's **single parent** — one position per entity, set here to place/move the node (not per feature/currency). **Tri-state:** *omit* = leave the parent unchanged (a new node defaults to root); `null` = make it a root; an *entity id* = set/change the parent. This payload is flat (one row per entity × feature-or-currency × scope), so the same entity across rows carries the same `parentId`. **Re-parenting is leaf-only** — moving a node that still has children is rejected; move/archive its children first.
- `featureId` **or** `currencyId` — What's limited — exactly one on the wire: a **`featureId`** (metered feature) **or** a **`currencyId`** (credit currency).
- `usageLimit` — The budget for one cadence window.
- `cadence` — A **single-unit** ISO-8601 duration for the reset window — `'P1M'` (monthly), `'P1D'` (daily), `'P1W'` (weekly), `'P1Y'` (yearly), or a single count like `'P10D'`. It must be one unit only — a composite like `'P1M10D'` (1 month *and* 10 days) is **not** allowed.
...
- The cadence must be a **single-unit** duration (`'P1M'`, `'P10D'`, …); a composite like `'P1M10D'` is rejected.
- Different assignments on the same entity can carry different cadences (daily cap + monthly cap on the same feature is a valid belt-and-suspenders pattern).
...
Leaf-only: if `proj-atlas` itself has children, this is rejected — re-parent or archive those children first. Wiring this into your restructure code path: `entity-lifecycle.md`.
...
- Passing both `featureId` and `currencyId` — An assignment limits one or the other, never both.
...
- Writing a composite cadence like `'P1M10D'` — Single-unit durations only; use `'P1M'` **or** `'P10D'`, not both in one cadence.
- Budgeting in USD via `currencyId` — Credit currencies only. Money budgets go through credit cost modeling (`stigg-credits`).
- Re-parenting a node with children — Leaf-only — rejected. Move/archive children first.
