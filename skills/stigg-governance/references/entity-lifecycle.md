...
> **What happens to an archived entity's unused budget?** Automatic "return the unused allocation to the parent" behavior is **not shipped today** — allocated/cascading budgets are on the governance roadmap (early access), where a limit set on an entity is drawn from a parent pool. Today a limit sits directly on the entity, so archiving simply stops that entity accruing; there is no verified re-distribution to the parent. Don't build on allocation-return until it ships.
