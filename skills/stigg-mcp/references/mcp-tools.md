...
MCP coverage is broad but not 100% of the Stigg API. When an operation isn't MCP-exposed, fall through to the SDK / REST API (skill: `stigg-api`). Use the Stigg CLI (`brew install stiggio/tools/stigg` — https://github.com/stiggio/stigg-cli) only when the user explicitly wants a CLI / scripted workflow.
